%%%%% build setting | 編譯設定 %%%%%
\setboolean{publish}{true} % {true}/{false} Set true before publish. 發怖前設true
...
\def\deptshort {電機}
\def\dept      {電機研究所}%XX學系XXX碩士班(請參考「中央大學學位論文撰寫體例參考」附錄)
\def\degree    {碩士} % Master(碩士)/Ph.D(博士)
\def\titleZh   {中興大學論文樣板(英文)}   % Chinese title
...
\def\degreedateZh{中~華~民~國~一一二~年~七~月} % Only for Chinese titlepage
...
\def\chapterTitleStyle{\sffamily\LARGE\bfseries} % cls default use \centering for zh
\def\sectionTitleStyle{\centering\sffamily\Large\bfseries}
\def\subsectionTitleStyle{\centering\sffamily\large\bfseries}
\def\subsubsectionTitleStyle{\centering\sffamily\normalsize\bfseries}
